...
static void doinclude(int silent)
{
  ...
  if (pc_compat) {
	...
	strcpy(symincname,"_inc_");
	if((ptr=strrchr(name,dirsep))!=NULL)
		strlcat(symincname,ptr+1,sizeof symincname);
	else
		strlcat(symincname,name,sizeof symincname);
    //included=find_symbol(&glbtab,symname,fcurrent,-1,NULL)!=NULL;
	if(Includes.find(std::string(symname))!=Includes.end()) 
	{
		included=1;
		FindIncludes=1;
	}
  ...
  if (!included) {
	...
	if(result && pc_compat)
	{
		add_constant(symincname,1,sGLOBAL,0);
		Includes.insert(std::string(symname));
	}
  ...
  case tpPRAGMA:
    if (!SKIPPING) {
      if (lex(&val,&str)==tSYMBOL) {
		...
		else if(strcmp(str,"disablerecursion")==0) DisableRecursion=1;
		else
		{
          error(207);           /* unknown #pragma */
        } /* if */
      ...
      if (lex(&val,&str)==tSYMBOL) {
        ...
        if (!ret) {
          ...
          if (sym!=NULL && (sym->usage & (uENUMROOT | uENUMFIELD))==0) {
			  if(strstr(sym->name,"_inc_")) Includes.erase(std::string(sym->name));
...
std::unordered_map<int,SymLinks> SymbolList;
...
static symbol *add_symbol(symbol *root,symbol *entry,int sort)
{
  ...
  if(sort)
  {
	  //if(lastroot) if(lastroot->next!=NULL) if(strcmp(entry->name,lastroot->next->name)>0) root=lastroot;
	  while(root->next!=NULL && strcmp(entry->name,root->next->name)>0) root=root->next;
	  //lastroot=root;
  }
  ...
  std::unordered_map<int,SymLinks>::iterator it=SymbolList.find((int)addroot);
  if(it!=SymbolList.end())
  {
	  SymLinks::iterator it2=it->second.find(std::string(entry->name));
	  if(it2!=it->second.end())
	  {
		  it2->second.insert((int)root->next);
	  }
	  else
	  {
		  //Add?
		  ...
		  SymLink SymLinkAdd;
		  SymLinkAdd.insert((int)root->next);

		  it->second.insert(std::make_pair(std::string(entry->name),SymLinkAdd));
	  }
  }
  else
  {
	  ...
	  SymLinksAdd.insert(std::make_pair(std::string(entry->name),SymLinkAdd));

	  SymbolList.insert(std::make_pair((int)addroot,SymLinksAdd));
  }

  return newsym;
}

static void free_symbol(symbol *root,symbol *sym)
{
  arginfo *arg;

  std::unordered_map<int,SymLinks>::iterator it=SymbolList.find((int)root);
  if(it!=SymbolList.end())
  {
	  ...
	  if(!it->second.size()) SymbolList.erase((int)root); //Erase?
  }
...
static symbol *find_symbol(const symbol *root,const char *name,int fnumber,int automaton,int *cmptag)
{
  ...
  //pc_printf("\nfind_symbol %s %d %d\n",name,cmptag,root==&glbtab);

  //if(allowbreak) 
  //if(root!=&glbtab)
  {
	  find=1;
	  //std::unordered_map<std::string,SymLinks>::iterator j=SymbolList.find(std::string(name));
	  //pc_printf("\nfind %d\n",SymbolList.size());
	  std::unordered_map<int,SymLinks>::iterator j=SymbolList.find((int)root);
	  if(j!=SymbolList.end())
	  {
		  //pc_printf("\nfind %s %d\n",name,cmptag);
		  ...
		  SymLinks::iterator it=j->second.find(std::string(name));
		  //if(it==j->second.end()) pc_printf("\n!find2 %s\n",name);
		  //int temp=0;
		  //if(it!=j->second.end()) if(it->second.size()) pc_printf("\nfind3 %d\n",it->second.size());
		  if(it!=j->second.end()) for(SymLink::iterator i=it->second.begin();i!=it->second.end();i++)
		  {
			  //pc_printf("\nfind2 %s\n",name);
			  ...
			  //sym=(symbol*)i->second;
			  sym=(symbol*)(*i);
			  //if(sym==NULL) pc_printf("\nsym NULL\n");
			  if(sym!=NULL)
			  {
				  //if(hash!=sym->hash || strcmp(name,sym->name)!=0) pc_printf("\nname %d %d %s %s %d\n",hash,sym->hash,(char*)name,sym->name,cmptag);
				  if(hash==sym->hash && strcmp(name,sym->name)==0 && (sym->parent==NULL || sym->ident==iCONSTEXPR) && (sym->fnumber<0 || sym->fnumber==fnumber))
				  {
					  assert(sym->states==NULL || sym->states->next!=NULL);
					  if(sym->ident==iFUNCTN || automaton<0 && sym->states==NULL || automaton>=0 && sym->states!=NULL && state_getfsa(sym->states->next->index)==automaton)
					  {
  ...
  {
	  if(*cmptag==0) *cmptag=count;
	  else *cmptag=1;
  }
  //if(cmptag!=NULL && firstmatch!=NULL) *cmptag=count;
  return firstmatch;
}

static symbol *find_symbol_child(const symbol *root,const symbol *sym)
{
	if(sym->unparent) if(sym->unparent->parent==sym) return sym->unparent;
	return NULL;
...
SC_FUNC symbol *addsym(const char *name,cell addr,int ident,int vclass,int tag,int usage)
{
  ...
  *refer=NULL;
  ...
  entry.numrefers=1;
  entry.refer=refer;
  entry.FileName=new char[260];
  memset(entry.FileName,0,260);
  sprintf(entry.FileName,inpfname);
  entry.IsRecursion=0;
