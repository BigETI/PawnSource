...
SC_FUNC void stgwrite(const char *st)
{
  ...
  if (staging) {
    ...
    //strcat(stgbuf,st);
	for(int i=len;i<len+lenst;i++) stgbuf[i]=st[i-len];
	stgbuf[len+lenst]=0;
	stglength=len+lenst;
    //len=strlen(stgbuf);
	len+=lenst;
...
typedef std::unordered_map<char,idlist> sequenceslist2;
std::unordered_map<char,sequenceslist2> sequenceslist;
...
SC_FUNC int phopt_init(void)
{
	...
	for(number=0; sequences[number].find!=NULL; number++) number++;             /* include an item for the NULL terminator */

	sequenceslenght=new int[number];

	for(int i=0; i<number; i++) sequenceslenght[i]=0;

	int stop=0;
	for(int i=0; i<number-1; i++) if(sequences[i].find!=NULL)
	{
		sequenceslenght[i]=strlen(sequences[i].find);

		char str[160];
		memset(str,0,160);
		strcpy(str,sequences[i].find);
		if(str[0]==0) stop=1;
		if(!stop)
		{
			std::unordered_map<char,sequenceslist2>::iterator it=sequenceslist.find(str[0]);
			if(it!=sequenceslist.end())
			{
				sequenceslist2::iterator it2=it->second.find(str[1]);
				if(it2!=it->second.end()) it2->second.insert(i);
				else
				{
					idlist SymLink;
					SymLink.insert(i);

					it->second.insert(std::make_pair(str[1],SymLink));
				}
			}
			else
			{
				...
				sequenceslist2 SymLink2;
				SymLink2.insert(std::make_pair(str[1],SymLink));

				sequenceslist.insert(std::make_pair(str[0],SymLink2));
			}
		}
	}
...
static int matchsequence(char *start,char *end,char *pattern,char symbols[MAX_OPT_VARS][MAX_ALIAS+1],int *match_length,int length)
{
  int var,i;
  ...
  if(end-start-2<length) return 0;
  ...
  *match_length=0;
  ...
  while (*pattern) {
    ...
    case '%':   /* new "symbol" */
	  ...
	  for(i=0;start<end && (*start=='-' || *start=='+' ||
		  ((*start>='a' && *start<='z') || (*start>='A' && *start<='Z') || *start=='_' || *start==PUBLIC_CHAR || (*start>='0' && *start<='9'))
		  //alphanum(*start)
		  );
	  i++,start++)
	  {
    ...
    case '!':
      ...
      break;
    default:
		if(*start!=*pattern) //if(tolower(*start) != tolower(*pattern))
        return FALSE;
      start++;
    } /* switch */
    pattern++;
...
static void stgopt(char *start,char *end,int (*outputfunc)(char *str))
{
	...
	if(pc_optimize>sOPTIMIZE_NONE && sc_status==statWRITE)
	{
		do
		{
			...
			if((*start)!='\t') while(end-start>=1 && !(((*start)>='a' && (*start)<='z') || (*start)=='\t' || (*start)==' ' || (*start)==';' || (*start)=='$')) start++;
			while(*start=='\t' || *start==' ') start++;
			while(start<end)
			{
				seq=0;
				...
				std::unordered_map<char,sequenceslist2>::iterator it=sequenceslist.find(*start);
				if(it!=sequenceslist.end())
				{
					std::unordered_map<char,idlist>::iterator it2=it->second.find(*(start+1));
					if(it2!=it->second.end()) for(idlist::iterator i=it2->second.begin();i!=it2->second.end();i++)
					{
						seq=(*i);
						if(sequences[seq].find!=NULL)
						{
							if(matchsequence(start,end,sequences[seq].find,symbols,&match_length,sequenceslenght[seq]))
							{
				...
				start+=strlen(start) + 1;
				if(end-start>3) if(*start=='\t') if(*(start+1)==';') if(*(start+2)=='$') if(*(start+3)!='l') start+=strlen(start) + 1;
				while(*start=='\t' || *start==' ') start++;
				if((*start)!='\t') while(end-start>=1 && !(((*start)>='a' && (*start)<='z') || (*start)=='\t' || (*start)==' ' || (*start)==';' || (*start)=='$')) start++;
				while(*start=='\t' || *start==' ') start++;
			}
	...
	for(start=debut; start<end; start+=strlen(start)+1) outputfunc(start);
}
