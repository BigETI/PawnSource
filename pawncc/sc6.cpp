...
static cell do_dump(FILE *fbin,char *params,cell opcode)
{
  ...
  if(strlen(params)>=7)
  {
	  if(params[0]=='c' && params[1]=='o' && params[2]=='u' && params[3]=='n' && params[4]=='t')
	  {
		  params+=6;
		  num=getparam(params,&params);


		  if(!InitNull)
		  {
			  Nullst=new char[2000];
			  memset(Nullst,0,2000);
			  InitNull=1;
		  }
		  //char *Nullst=new char[num];
		  //memset(Nullst,0,num);
		  //for(int i=0;i<num;i++) writeerror|=!pc_writebin(fbin,&code,1);
		  writeerror|=!pc_writebin(fbin,Nullst,num);

		  return num*sizeof(cell);
	  }

  }
  if(!find) while(*params!='\0')
  {
	  //pc_printf("\nparams %s\n",params);
	  if(params[0]!='0')
	  {
	  ...
	  p=0;
	  params++;
	  //if(fbin!=NULL) write_encoded(fbin,&p,1);
	  if(!find)
	  {
		  find=1;
		  findid=p;
	  }
	  else if(findid!=p) stop=1;
	  num++;
  ...
  if(!stop)
  {
	  ...
	  char *st=new char[num];
	  memset(st,0,num);
	  //for(int i=0;i<num;i++) writeerror|=!pc_writebin(fbin,&code,1);
	  writeerror|=!pc_writebin(fbin,st,num);
  }
  else
  {
	  params=start;
...
static int findopcode(char *instr,int maxlen)
{
  ...
  if(strcmp(str,"dump")==0) return 26;
