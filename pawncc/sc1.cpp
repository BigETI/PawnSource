...
void pc_printf_rus(const char *message,...)
{
	FILE *f=fopen(StartDir,"a+");
	if(f)
	{
		va_list argptr;
		va_start(argptr,message);
		...
		std::string utf8_string=convert.to_bytes(strtowstr(std::string(message)).c_str());

		vfprintf(f,utf8_string.c_str(),argptr);
		fclose(f);
	}
}
...
static char *prefix[3]={ "error", "fatal error", "warning" };
  ...
  if (number!=0) {
	...
	if(number==217 || number==203 || number==204)
	{
		if(firstline>=0) fprintf(stderr,"%s(%d) -- (%d) : ",filename,firstline,lastline);
		else fprintf(stderr,"%s(%d) : ",filename,lastline);
	}
	else
	{
		if(firstline>=0) fprintf(stderr,"%s(%d) -- (%d) : %s %03d: ",filename,firstline,lastline,pre,number);
		else fprintf(stderr,"%s(%d) : %s %03d: ",filename,lastline,pre,number);
	}
  } /* if */
...
int pc_error_rus(int number,char *message,char *filename,int firstline,int lastline,va_list argptr)
{
	static char *prefix[3]={"ошибка","фатальная ошибка","предупреждение"};
	...
	FILE *f=fopen(StartDir,"a+");
	{
		...
		if(number!=0)
		{
			std::string utf8_string=convert.to_bytes(strtowstr(std::string(filename)).c_str());

			char *pre=prefix[number/100];
			std::string utf8_prefix=convert.to_bytes(strtowstr(std::string(pre)).c_str());
			if(number==217 || number==203 || number==204)
			{
				if(firstline>=0) fprintf(f,"%s(%d) -- (%d) : ",utf8_string.c_str(),firstline,lastline);
				else fprintf(f,"%s(%d) : ",utf8_string.c_str(),lastline);
			}
			else
			{
				if(firstline>=0) fprintf(f,"%s(%d) -- (%d) : %s %03d: ",utf8_string.c_str(),firstline,lastline,utf8_prefix.c_str(),number);
				else fprintf(f,"%s(%d) : %s %03d: ",utf8_string.c_str(),lastline,utf8_prefix.c_str(),number);
			}
		} /* if */
...
int pc_compile(int argc, char *argv[])
{
  ...
  //else if (verbosity>0) setcaption();
  ...
  if (get_sourcefile(1)!=NULL) {
    ...
    FILE *ftmp,*fsrc;
    int fidx;
    ftmp=(FILE*)pc_createtmpsrc(&tname);
...
cleanup:
  ...
  if(FindIncludes && IncludePossibleBug && errnum && strlen(errfname)==0)
  {
	  pc_printf("Enable y_hooks include several times: #pragma compat 0\n",(long)hdrsize);
	  pc_printf_rus("Для включения подключения y_hooks несколько раз: #pragma compat 0\n",(long)hdrsize);
  }
    ...
    if (errnum==0 && strlen(errfname)==0) {
	  ...
	  if(FindRecursion)
	  {
		  pc_printf("Disable recursion warning: #pragma disablerecursion or #pragma warning disable 238\n",(long)hdrsize);
		  pc_printf_rus("Для отключения вывода рекурсий: #pragma disablerecursion или #pragma warning disable 238\n",(long)hdrsize);
	  }
	  if(!strlen(errfname)) if(verbosity>0)
	  {
		  setcaption();
		  SetCaption=1;
	  }
      ...
      if (/*(sc_debug & sSYMBOLIC)!=0 || */verbosity>=2 || stacksize+32>=(long)pc_stksize || flag_exceed) {
		...
		pc_printf_rus("Размер заголовка:\t%8ld байт\n",(long)hdrsize);
		pc_printf_rus("Размер кода:\t\t%8ld байт\n",(long)code_idx);
		pc_printf_rus("Размер данных:\t\t%8ld байт\n",(long)glb_declared*sizeof(cell));
		pc_printf_rus("Размер стэка\\кучи:\t%8ld байт; ",(long)pc_stksize*sizeof(cell));
		pc_printf_rus("Примерное макс. использование");
		if(recursion) pc_printf_rus(": Неизвестно, из-за рекурсий\n");
		else if((pc_memflags & suSLEEP_INSTR)!=0) pc_printf_rus(": Неизвестно, из-за использования \"sleep\" \n");
		else pc_printf_rus("=%ld ячеек (%ld байт)\n",stacksize,stacksize*sizeof(cell));
		pc_printf_rus("Всего требуется:\t%8ld байт\n",(long)hdrsize+(long)code_idx+(long)glb_declared*sizeof(cell)+(long)pc_stksize*sizeof(cell));
      } /* if */
      if (flag_exceed)
	...
	if(!SetCaption) if(!strlen(errfname)) if(verbosity>0) setcaption();
  ...
  else if(warnnum+recurnum!=0)
  {
	  if(strlen(errfname)==0)
	  {
		  pc_printf("\n%d Warning%s.\n",warnnum+recurnum,(warnnum+recurnum>1) ? "s" : "");
		  pc_printf_rus("\nПредупреждений:%d.\n",warnnum+recurnum);
	  }
	  retcode=2;          /* use "0", so that MAKE and similar tools continue */
  }
  else
  {
	  retcode=jmpcode;
    ...
    if (isoption) {
      ...
      case 'w':
		...
		if(*ptr=='-')
		{
			...
			if(i==238) DisableRecursion=1;
		}
		else if(*ptr=='+')
		{
			pc_enablewarning(i,1);
			if(i==238) DisableRecursion=0;
		}
		else if(*ptr=='\0')
		{
			pc_enablewarning(i,2);
			if(i==238) DisableRecursion=!DisableRecursion;
		}
        break;
      case 'X':
        if (*(ptr+1)=='D') {
...
static void setcaption(void)
{
  pc_printf("Pawn compiler " VERSION_STR "\t \t \tCopyright (c) 1997-2017, ITB CompuPhase\n\n");
  pc_printf_rus("Pawn compiler " VERSION_STR "\t \t \tCopyright (c) 1997-2017, ITB CompuPhase\n\n");
}
...
static void about(void)
{
  if (strlen(errfname)==0) {
	...
	pc_printf_rus("Usage:   pawncc <filename> [filename...] [options]\n\n");
	pc_printf_rus("Options:\n");
	pc_printf_rus("         -A<num>  alignment in bytes of the data segment and the stack\n");
	pc_printf_rus("         -a       output assembler code\n");
#if AMX_COMPACTMARGIN > 2
	pc_printf_rus("         -C[+/-]  compact encoding for output file (default=%c)\n",sc_compress ? '+' : '-');
#endif
	pc_printf_rus("         -c<name> codepage name or number; e.g. 1252 for Windows Latin-1\n");
	pc_printf_rus("         -Dpath   active directory path\n");
	pc_printf_rus("         -d<num>  debugging level (default=-d%d)\n",sc_debug);
	pc_printf_rus("             0    no symbolic information, no run-time checks\n");
	pc_printf_rus("             1    run-time checks, no symbolic information\n");
	pc_printf_rus("             2    full debug information and dynamic checking\n");
	pc_printf_rus("             3    same as -d2, but implies -O0\n");
	pc_printf_rus("         -e<name> set name of error file (quiet compile)\n");
#if defined	__WIN32__ || defined _WIN32 || defined _Windows
	pc_printf_rus("         -H<hwnd> window handle to send a notification message on finish\n");
#endif
	pc_printf_rus("         -i<name> path for include files\n");
	pc_printf_rus("         -l       create list file (preprocess only)\n");
	pc_printf_rus("         -o<name> set base name of (P-code) output file\n");
	pc_printf_rus("         -O<num>  optimization level (default=-O%d)\n",pc_optimize);
	pc_printf_rus("             0    no optimization\n");
	pc_printf_rus("             1    JIT-compatible optimizations only\n");
	pc_printf_rus("             2    full optimizations\n");
	pc_printf_rus("         -p<name> set name of \"prefix\" file\n");
#if !defined SC_LIGHT
	pc_printf_rus("         -r[name] write cross reference report to console or to specified file\n");
#endif
	pc_printf_rus("         -S<num>  stack/heap size in cells (default=%d)\n",(int)pc_stksize);
	pc_printf_rus("         -s<num>  skip lines from the input file\n");
	pc_printf_rus("         -t<num>  TAB indent size (in character positions, default=%d)\n",sc_tabsize);
	pc_printf_rus("         -v<num>  verbosity level; 0=quiet, 1=normal, 2=verbose (default=%d)\n",verbosity);
	pc_printf_rus("         -w<num>  disable a specific warning by its number\n");
	pc_printf_rus("         -X<num>  abstract machine size limit in bytes\n");
	pc_printf_rus("         -XD<num> abstract machine data/stack size limit in bytes\n");
	pc_printf_rus("         -Z[+/-]  run in compatibility mode (default=%c)\n",pc_compat ? '+' : '-');
	pc_printf_rus("         -\\       use '\\' for escape characters\n");
	pc_printf_rus("         -^       use '^' for escape characters\n");
	pc_printf_rus("         -;[+/-]  require a semicolon to end each statement (default=%c)\n",sc_needsemicolon ? '+' : '-');
	pc_printf_rus("         -([+/-]  require parantheses for function invocation (default=%c)\n",optproccall ? '-' : '+');
	pc_printf_rus("         sym=val  define constant \"sym\" with value \"val\"\n");
	pc_printf_rus("         sym=     define constant \"sym\" with value 0\n");
#if defined	__WIN32__ || defined _WIN32 || defined _Windows || defined __MSDOS__
	pc_printf_rus("\nOptions may start with a dash or a slash; the options \"-d0\" and \"/d0\" are\n");
	pc_printf_rus("equivalent.\n");
#endif
	pc_printf_rus("\nOptions with a value may optionally separate the value from the option letter\n");
	pc_printf_rus("with a colon (\":\") or an equal sign (\"=\"). That is, the options \"-d0\", \"-d=0\"\n");
	pc_printf_rus("and \"-d:0\" are all equivalent.\n");
  } /* if */
...
static void setconstants(void)
{
  ...
  lineparse=add_constant("__line",0,sGLOBAL,0);
...
static void dumpzero(int count)
{
  ...
  /*if(count>=16) //time? some bug
  {
  char st1[256];
  sprintf(st1,"dump count %X",count);
  stgwrite(st1);
  count=0;
  }*/
  while(count>=2000)
  {
	  stgwrite("dump count 7D0\n");
	  count-=2000;
  }
  while(count>=1024)
  {
	  stgwrite("dump count 400\n");
	  count-=1024;
  }
  while(count>=1000)
  {
	  stgwrite("dump count 3E8\n");
	  count-=1000;
  }
  while(count>=512)
  {
	  stgwrite("dump count 200\n");
	  count-=512;
  }
  while(count>=256)
  {
	  stgwrite("dump count 100\n");
	  count-=256;
  }
  while(count>=128)
  {
	  stgwrite("dump count 80\n");
	  count-=128;
  }
  while(count>=64)
  {
	  stgwrite("dump count 40\n");
	  count-=64;
  }
  while(count>=32)
  {
	  stgwrite("dump count 20\n");
	  count-=32;
  }
  while(count>=16)
  {
	  stgwrite("dump count 10\n");
	  count-=16;
  }
  ...
  if(count)
  {
	  ...
	  while(count-->0)
	  {
		  ...
		  //stgwrite("0");
		  i=(i+1) % 16;
		  if(i==0 || count==0) stgwrite("0\n");
		  else stgwrite("0 ");
		  //stgwrite((i==0 || count==0) ? "\n" : " ");
		  if(i==0 && count>0) defstorage();
	  }
  }
}
  ...
  if (ident==iVARIABLE) {
    ...
    if (numdim==1) {
      *size=initvector(ident,tag,dim[0],litidx,FALSE,enumroot,NULL);
      ...
      int idx;
	  constvalue lastdim={NULL,"",0,NULL,0};     /* sizes of the final dimension */
      int skipdim=0;
...
static void decl_enum(int vclass)
{
  ...
  do {
    ...
    sym->parent=enumsym;
	if(enumsym) enumsym->unparent=sym;
...
static void attachstatelist(symbol *sym, int state_id)
{
  ...
  {
	  if(sym->ident==iFUNCTN) if(!(errflag || sc_status!=statWRITE)) if(strstr(sym->name,"@yH_")) IncludePossibleBug=1;
...
SC_FUNC symbol *fetchfunc(char *name,int tag)
{
  ...
  if ((sym=findglb(name,sGLOBAL))!=0) 
  {   /* already in symbol table? */
	  sym->lnumber=fline;
	  memset(sym->FileName,0,260);
	  sprintf(sym->FileName,inpfname);
...
typedef std::unordered_set<int> SymLink;
typedef std::unordered_map<std::string,SymLink> SymLinks;
extern std::unordered_map<int,SymLinks> SymbolList;
...
static int operatoradjust(int opertok,symbol *sym,char *opername,int resulttag)
{
  ...
  std::unordered_map<int,SymLinks>::iterator it=SymbolList.find((int)&glbtab);
  if(it!=SymbolList.end())
  {
	  SymLinks::iterator it2=it->second.find(std::string(sym->name));
	  if(it2!=it->second.end())
	  {
		  it2->second.erase((int)sym);
		  if(!it2->second.size()) it->second.erase(std::string(sym->name)); //Erase?
	  }

	  if(!it->second.size()) SymbolList.erase((int)&glbtab); //Erase?
  }

  strcpy(sym->name,tmpname);

  it=SymbolList.find((int)&glbtab);
  if(it!=SymbolList.end())
  {
	  SymLinks::iterator it2=it->second.find(std::string(sym->name));
	  if(it2!=it->second.end())
	  {
		  it2->second.insert((int)sym);
	  }
	  else
	  {
		  //Add?
		  ...
		  SymLink SymLinkAdd;
		  SymLinkAdd.insert((int)sym);

		  it->second.insert(std::make_pair(std::string(sym->name),SymLinkAdd));
	  }
  }
  else
  {
	  ...
	  SymLinks SymLinksAdd;
	  SymLinksAdd.insert(std::make_pair(std::string(sym->name),SymLinkAdd));

	  SymbolList.insert(std::make_pair((int)&glbtab,SymLinksAdd));
  }
    ...
    do {                                /* there are arguments; process them */
      ...
      case '&':
        if (ident!=iVARIABLE || numtags>0)
          error(1,"-identifier-","&"); //Руссификация?
        ident=iREFERENCE;
        break;
      case tCONST:
        if (ident!=iVARIABLE || numtags>0)
      ...
      for (stkpos=0; sourcesym[stkpos]!=NULL; stkpos++) {
        if (sym->refer[i]==sourcesym[stkpos]) {   /* recursion detection */
          *recursion=1;
		  ...
		  if(!DisableRecursion) if(!sym->IsRecursion)
		  {
			  if(sourcesym[stkpos]==sym) pc_printf_rus("%s(%d) : предупреждение: рекурсия в %s\n",sym->FileName,sym->lnumber,sym->name);
			  else pc_printf_rus("%s(%d) : предупреждение: рекурсия в %s: вызов с %s %s(%d)\n",sym->FileName,sym->lnumber,sym->name,sourcesym[stkpos]->name,sourcesym[stkpos]->FileName,sourcesym[stkpos]->lnumber);

			  if(sourcesym[stkpos]==sym) pc_printf("%s(%d) : warning: recursion in %s\n",sym->FileName,sym->lnumber,sym->name);
			  else pc_printf("%s(%d) : warning: recursion in %s: call from %s %s(%d)\n",sym->FileName,sym->lnumber,sym->name,sourcesym[stkpos]->name,sourcesym[stkpos]->FileName,sourcesym[stkpos]->lnumber);

			  recurnum++;
			  FindRecursion=1;
		  }

		  sourcesym[stkpos]->IsRecursion=1;
		  sym->IsRecursion=1;
  ...
  while (sym!=NULL && sym->compound>=level) {
    ...
    default:
      ...
      if ((sym->usage & (uWRITTEN | uREAD | uSTOCK))==0) {
		  ...
		  if(sym->FileName)
		  {
			  int len=strlen(inpfname);
			  temp=new char[len+1];
			  memset(temp,0,len+1);
			  sprintf(temp,inpfname);
			  sprintf(inpfname,sym->FileName);
		  }
		...
		if(sym->FileName)
		{
			sprintf(inpfname,temp);
			delete temp;
		}
		//pc_printf_rus("\n\n%s -> %s\n\n",sym->name,sym->FileName);
...
{
  constvalue *cur,*prev;
  constvalue *tablestart=table;
  if(tablestart->end)
  {
	  prev=tablestart->end;
	  cur=tablestart->end->next;
  }
  else
  {
	  prev=table;
...
static void doswitch(void)
{
  ...
  cell val;
  char *str;
  constvalue caselist={NULL,"",0,NULL,0};   /* case list starts empty */
  constvalue *cse,*csp;
